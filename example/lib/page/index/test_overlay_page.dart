...
class _TestOverlayPageState extends State<TestOverlayPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('测试弹出层'),),
      body: WeCellGroup(
        children: <WeCell>[
          WeCell('modal',
            onTap: () {
              showModal(title: '提示窗', onCancel: () {
                showToast('取消');
              }, onConfirm: () {
                showToast('确定');
              });
          }),
          WeCell('toast原生的（推荐使用）',
            onTap: () {
              showToast('toast原生的');
          }),
          WeCell('toast弹出层的',
          ...
          WeCell('previewImage',
            onTap: () {
              previewImage(
                current: 'https://i0.hdslb.com/bfs/article/c220c9dd67c5df555cd8634abf0a06fd07ef1e44.jpg@1320w_882h.webp',
                urls: [
