...
class _TestRouterPageState extends State<TestRouterPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('测试路由${widget.pageCount}'),),
          ...
          children: <Widget>[
            WeCellGroup(
              children: <WeCell>[
                ...
                WeCell('测试返回1层', onTap: () {
                  Router.navigateBack(delta: 1, result: '我从${widget.pageCount}页面返回过来的');
                }),
                WeCell('测试返回3层', onTap: () {
                  Router.navigateBack(delta: 3, result: '我从${widget.pageCount}页面返回过来的');
                }),
              ],
            ),
            ...
            WeCellGroup(
              title: 'url模式（可带参数）',
              children: <WeCell>[
                WeCell('用url模式跳转带参数的一页', onTap: () {
                  Router.navigateTo(
                    url: '/test_router',
                    params: {
                      'id': widget.pageCount + 1
                    }
                  ).then((val) {
                    /// 从前一个页面返回的值输出
                    showToast('$val \n 当前页面为第${widget.pageCount}页');
